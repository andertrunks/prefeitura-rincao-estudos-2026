...
import { defaultUserData, loadUserData, saveUserData, validateImportedData } from './storage'
import type {
  AnswerRecord,
  CargoId,
  DisciplineId,
...
function Header({ data, setData }: AppState) {
      ...
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <select
            ...
            onChange={(event) => setData((current) => ({ ...current, selectedCargo: event.target.value as CargoId }))}
          >
...
function Sidebar({ data }: AppState) {
  const cargo = cargos.find((item) => item.id === data.selectedCargo)!
...
function AppLayout({ data, setData }: AppState) {
  return (
...
function Dashboard({ data }: AppState) {
  const cargo = cargos.find((item) => item.id === data.selectedCargo)!
  const cargoTopics = byCargo(data.selectedCargo)
  const cargoQuestions = questions.filter((question) => question.cargoId === data.selectedCargo)
  const latest = latestAnswers(data.answers.filter((answer) => cargoQuestions.some((question) => question.id === answer.questionId)))
  const completed = cargoTopics.filter((topic) => data.completedTopics.includes(topic.id)).length
  const coverage = Math.round((completed / cargoTopics.length) * 100)
  const disciplineStats = (Object.keys(disciplineLabels) as DisciplineId[]).map((discipline) => {
    const ids = cargoQuestions.filter((question) => question.discipline === discipline).map((question) => question.id)
    const records = latest.filter((record) => ids.includes(record.questionId))
    return { discipline, rate: accuracy(records), count: records.length }
  })
  const weakest = disciplineStats.filter((item) => item.count).sort((a, b) => a.rate - b.rate)[0]
    ...
    <>
      <section className="dashboard-hero">
        <div>
          ...
          <p>Plano gratuito para {cargo.name}, com estudo, questões e progresso salvos neste dispositivo.</p>
      ...
      <section className="dashboard-grid">
        ...
        <article className="metric-card"><span>Revisões agendadas</span><strong>{data.reviews.length}</strong><small>{data.reviews.filter((item) => new Date(item.dueAt) <= new Date()).length} para revisar agora</small></article>
      </section>

      <section className="dashboard-columns">
        <article className="panel recommendation">
          <div className="panel-title"><Target size={20} /><div><span>Recomendação de estudo</span><h2>{weakest ? `Reforce ${disciplineLabels[weakest.discipline]}` : 'Comece pelo próximo tópico'}</h2></div></div>
          <p>{weakest ? `Seu aproveitamento atual é ${weakest.rate}%. Faça uma revisão direcionada antes de outro simulado.` : 'Responda algumas questões para receber recomendações baseadas no seu desempenho real.'}</p>
          <Link to={weakest ? `/questoes?disciplina=${weakest.discipline}` : nextTopic ? `/aula/${nextTopic.id}` : '/estudo'}>Abrir recomendação <ArrowRight size={17} /></Link>
        </article>
      ...
      <div className="cargo-grid full">
        {cargos.map((cargo) => {
          ...
          return <article key={cargo.id} className={`cargo-card ${cargo.color} ${selected ? 'selected' : ''}`}>
            ...
            <div className="card-actions"><button onClick={() => setData((current) => ({ ...current, selectedCargo: cargo.id }))}>{selected ? <><Check size={17} /> Selecionado</> : 'Selecionar cargo'}</button><Link to="/estudo">Ver conteúdo <ArrowRight size={16} /></Link></div>
          </article>
        })}
      </div>
    </>
...
function LessonPage({ data, setData }: AppState) {
  ...
  const relatedQuestions = questions.filter((question) => question.cargoId === data.selectedCargo && question.topicId === topic.id)
  ...
  const scheduleReview = (days: 1 | 7 | 15 | 30) => setData((current) => ({ ...current, reviews: [...current.reviews.filter((review) => review.topicId !== topic.id), { topicId: topic.id, intervalDays: days, dueAt: new Date(Date.now() + days * 86400000).toISOString() }] }))
...
function QuestionsPage({ data, setData }: AppState) {
  ...
  const latest = latestAnswers(data.answers)
  const filtered = questions.filter((question) => {
    if (question.cargoId !== data.selectedCargo) return false
  ...
  const confirm = () => {
    ...
    const record: AnswerRecord = { questionId: question.id, selectedIndex: selected, correct: selected === question.correctIndex, answeredAt: new Date().toISOString() }
...
function SimulationsPage({ data, setData }: AppState) {
  ...
  const set = useMemo(() => questions.filter((question) => question.cargoId === data.selectedCargo), [data.selectedCargo])
  ...
  const finish = (finalAnswers = answers) => {
    ...
    const records: AnswerRecord[] = set.map((question) => ({ questionId: question.id, selectedIndex: finalAnswers[question.id] ?? -1, correct: finalAnswers[question.id] === question.correctIndex, answeredAt: now, simulationId }))
    const simulation: SimulationRecord = { id: simulationId, cargoId: data.selectedCargo, mode, questionIds: set.map((question) => question.id), answerIds: records.map((record) => record.questionId), startedAt: new Date(Date.now() - seconds * 1000).toISOString(), finishedAt: now, elapsedSeconds: seconds }
...
function ErrorNotebook({ data }: AppState) {
  const cargoIds = questions.filter((question) => question.cargoId === data.selectedCargo).map((question) => question.id)
  const errors = latestAnswers(data.answers).filter((record) => cargoIds.includes(record.questionId) && !record.correct)
...
function ReviewsPage({ data, setData }: AppState) {
  const reviews = data.reviews.filter((review) => topics.some((topic) => topic.id === review.topicId && topic.cargoIds.includes(data.selectedCargo))).sort((a, b) => a.dueAt.localeCompare(b.dueAt))
  const remove = (topicId: string) => setData((value) => ({ ...value, reviews: value.reviews.filter((review) => review.topicId !== topicId) }))
...
function PerformancePage({ data, setData }: AppState) {
  ...
  const cargoTopics = byCargo(data.selectedCargo)
  const cargoQuestions = questions.filter((question) => question.cargoId === data.selectedCargo)
  const latest = latestAnswers(data.answers).filter((record) => cargoQuestions.some((question) => question.id === record.questionId))
  const exportData = () => { const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `rincao-estudos-backup-${new Date().toISOString().slice(0, 10)}.json`; anchor.click(); URL.revokeObjectURL(url) }
  const importData = async (file?: File) => { if (!file) return; try { const value = JSON.parse(await file.text()); if (!validateImportedData(value)) throw new Error(); setData(value); alert('Dados importados com sucesso.') } catch { alert('O arquivo não é um backup válido do Rincão Estudos.') } }
  return <><PageHeader eyebrow="Desempenho" title="Seu progresso real" description="Indicadores calculados apenas com seu histórico neste dispositivo." />
    ...
    <section className="data-panel"><div><Download size={24} /><span><h2>Seus dados são seus</h2><p>Exporte um backup antes de trocar de computador ou limpar o navegador.</p></span></div><div><button onClick={exportData}><Download size={17} /> Exportar meus dados</button><button onClick={() => fileRef.current?.click()}><Upload size={17} /> Importar meus dados</button><input ref={fileRef} hidden type="file" accept="application/json" onChange={(event) => importData(event.target.files?.[0])} /></div></section>
  </>
}
...
function Footer() {
  return <footer className="site-footer"><div><GraduationCap size={20} /><span><strong>Rincão Estudos 2026</strong><small>Projeto gratuito, educacional e não oficial.</small></span></div><p>Dados do edital auditados em 21/08/2026. Verifique convocações e retificações nos canais oficiais da Prefeitura e da INEPAM.</p></footer>
}
...
function App() {
  ...
  return <HashRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }}><AppLayout data={data} setData={setData} /></HashRouter>
}
